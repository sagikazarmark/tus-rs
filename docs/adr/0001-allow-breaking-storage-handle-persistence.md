...
OpenDAL storage uses `{prefix}/{upload_id}` as the main object key. PATCH bodies are staged under `{key}.parts/{part-number}` until the upload completes; completion materializes the main object through a temporary key and then cleans staging objects. The handle persists `opendal_next_part` as a cursor hint and `opendal_staged_size` as an accumulated size hint, written together after every accepted part. Recovery does not trust these hints as authoritative: `size` inspects staged objects and main-object metadata, and an append verifies the cursor's part object does not already exist before trusting the persisted size, falling back to listing staged objects when the hints are stale or missing, so persisted handles from before a crash do not overwrite accepted parts.
