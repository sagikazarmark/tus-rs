...
| `src/ui.rs` | Presentation-only helpers (`PageHeader`, `ExampleSection`, `SourceBlock`, formatters). |
...
| `/resume` | Advanced | `scan_resumable()` + `resume_entry()` — resume across a tab reload |
| `/options` | Advanced | `TusConfig` (chunk size, retries) + per-upload `TusStartOptions` (token, metadata) |
...
→ `http://localhost:8080/files`. The header's endpoint switcher re-points the
...
Start a TUS server on port 8080:
...
Open `http://localhost:8081`. Set the upload endpoint from the header switcher,
or bake a default in with `TUS_ENDPOINT=http://localhost:8080/files dx serve`.
...
Then open `http://localhost:8081` — the demo (`serve`) runs there and uploads to
this repo's `tus-server` (`server`) on `localhost:8080`. The two ports differ on
